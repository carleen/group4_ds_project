...
def bulkStoreListings():
    ...
    for f in fnames:
        temp_listings = f'./datasets/primary/{f}/listings.csv'
        temp_reviews = f'./datasets/primary/{f}/reviews-2.csv'
        temp_calendar = f'./datasets/primary/{f}/calendar.csv'
        storeListings(temp_listings)
        storeReviews(temp_reviews)
        storeCalendar(temp_calendar)
    weather_listing = f'./datasets/secondary/weather.csv'
...
if __name__ == "__main__":
    xls_path = './datasets/primary/airbnb_data_dict.xls'
    createSQLFile(xls_path)
    createDatabaseFile()
    bulkStoreListings()
    storePropertyInfo('./datasets/secondary/property_info.csv')
